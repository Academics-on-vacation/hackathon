<template>
  ...
  <div v-else class="flight-analytics-container">
    ...
    <div class="metrics-grid">
      ...
      <!-- В разделе metrics-grid, после карточки типа БПЛА -->
      <div class="metric-card" v-if="flight.bws_model">
        ...
        <div class="metric-content">
          <span class="metric-label">Модель БПЛА</span>
          <span class="metric-value">{{ flight.bws_model }}</span>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="section-card">
        ...
        <div class="flight-params-grid">
          ...
          <!-- В разделе flight-params-grid, после высоты полета -->
<div class="param-card" v-if="flight.dep_aerodrome_name">
  <div class="param-icon bg-green-50">
    <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"/>
    </svg>
  </div>
  <div class="param-content">
    <span class="param-label">Аэродром вылета</span>
    <span class="param-value">{{ flight.dep_aerodrome_name }}</span>
    <span class="param-code text-xs text-gray-500" v-if="flight.dep_aerodrome_code">{{ flight.dep_aerodrome_code }}</span>
  </div>
</div>

<div class="param-card" v-if="flight.arr_aerodrome_name">
  <div class="param-icon bg-red-50">
    <svg class="w-5 h-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
    </svg>
  </div>
  <div class="param-content">
    <span class="param-label">Аэродром посадки</span>
    <span class="param-value">{{ flight.arr_aerodrome_name }}</span>
    <span class="param-code text-xs text-gray-500" v-if="flight.arr_aerodrome_code">{{ flight.arr_aerodrome_code }}</span>
  </div>
</div>
          <div class="param-card">
...
.aerodrome-marker.departure {
  @apply bg-green-100 text-green-600;
}

.aerodrome-marker.arrival {
...
.aerodrome-type {
  @apply px-2 py-1 bg-gray-200 text-gray-700 rounded text-xs font-medium;
}
